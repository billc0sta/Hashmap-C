...
static inline void _HS_insert_next(struct _HS_Node *front,
                                   struct _HS_Node *back);
...
static uint32_t _HS_hash(const char *key, uint64_t key_typesize);
...
int HS_add_element(Hashmap *hs, void *key, void *val) {
  ...
  uint32_t hash = _HS_hash(key, ktsize) % hs->_reserved;
...
int HS_remove_element(Hashmap *hs, void *key) {
  if (!hs || !key) {
    _HS_report_error("remove_element() failed", "invalid NULL ptr");
    return -1;
  }
  ...
  if (is_parent && !is_child) {
    uint32_t hash = _HS_hash(key, hs->_key_typesize) % hs->_reserved;
    hs->_arr[hash] = next;
  }
  --hs->_count;
...
static uint32_t _HS_hash(const char *key, uint64_t key_size) {
...
static void _HS_reserve_more(Hashmap *hs, uint64_t size) {
  ...
  for (int i = 0; i < hs->_count; ++i, current = current->next) {
    current->is_child  = 0;
    current->is_parent = 0;
    uint32_t hash = _HS_hash(current->key, hs->_key_typesize) % hs->_reserved;
